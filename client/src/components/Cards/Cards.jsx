      ...
      <div class="row">
        <div class="card-colums">
          <div class="card h-100 w-50 mx-auto bg-dark">
            <Link class="text-succes" key={id} to={`/products/${id}`}>
              <img
                src={image}
                style={{ height: "220px" }}
                class="card-img-top img-fluid bg-light"
                alt="img not found"
              ></img>
            </Link>
            <h4 class="lead">{model}</h4>
            <button className="stock">EN STOCK</button>
            <div class="card-body text-light">
              <h5 class="card-title text-decoration-none">{brand}</h5>
              <h6 class="lead">
                {inches + '" - '} {operative_system + " -"}{" "}
                {capacity < 10 ? capacity + "TB" : capacity + "GB"}
              </h6>
              <div class="card-title">
                <h3>
                  {acomodarPrecio(price)}
                </h3>
              </div>
            </div>
          </div>
